 ...
 */
public class Sqlite_modActivity extends Activity implements OnClickListener {
	private Button btn_addOne, btn_deleteone, btn_check, btn_deleteTable,
			btn_edit, btn_newTable;
	private TextView tv;
	private SqliteHelper myOpenHelper;// 创建一个继承SQLiteOpenHelper类实例
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		myOpenHelper = new SqliteHelper(this);// 实例一个数据库辅助器
	...
	public void onClick(View v) {
		try {
			...
			if (v == btn_addOne) {// 添加数据
				...
				cv.put(SqliteHelper.TEXT, "测试新的数据");
				mysql.insert(SqliteHelper.TABLE_NAME, null, cv);
				...
				// ---------------------- 读写句柄来删除
				mysql.delete("himi", SqliteHelper.ID + "=1", null);
				// 第一个参数 需要操作的表名
			...
			// 备注4------
				...
				// ------------------------句柄方式来修改 -------------
				ContentValues cv = new ContentValues();
				cv.put(SqliteHelper.TEXT, "修改后的数据");
